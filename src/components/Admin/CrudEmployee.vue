<template>
  <div class="crud-container">
    ...
    <div class="cards-container">
      ...
      <div v-else-if="filteredUsers.length === 0" class="empty">Aucun employé trouvé</div>
      <div v-else v-for="user in paginatedUsers" :key="user.id" class="data-card">
        <div class="card-header">
          ...
          <div class="card-badges">
            <span class="badge badge-employee">{{ user.role }}</span>
            <span :class="['badge', (user.status || 'active') === 'active' ? 'badge-active' : 'badge-inactive']">
              {{ (user.status || 'active') === 'active' ? 'Actif' : 'Inactif' }}
            </span>
          </div>
...
const filteredUsers = computed(() => {
  return users.value.filter(user => {
    ...
    const matchesStatus = userStatus === currentFilter.value
